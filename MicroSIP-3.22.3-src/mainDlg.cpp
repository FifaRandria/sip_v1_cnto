...
#define MICROSIP_2FA_MAX_ATTEMPTS      3
#define MICROSIP_2FA_LOCK_MS           (5 * 60 * 1000)   // 5 minutes
#define MICROSIP_2FA_MIN_GAP_MS        2000               // 2 secondes

bool CmainDlg::CheckAdminAuth(const CString* accessPoint)
{
	DWORD now = GetTickCount64();

	// 1. Verrouillage actif (trop de tentatives echouees)
	if (m_2faLockUntil != 0 && now < m_2faLockUntil) {
		CString msg;
		msg.Format(_T("Access is locked due to too many failed attempts. Please wait %u seconds before trying again."),
			(m_2faLockUntil - now) / 1000 + 1);
		AfxMessageBox(msg, MB_ICONWARNING);
		return false;
	}

	// 2. Delai minimal de 2 secondes entre deux tentatives
	if (m_2faFirstAttempted && m_2faLastAttempt != 0 && (now - m_2faLastAttempt) < MICROSIP_2FA_MIN_GAP_MS) {
		DWORD wait = MICROSIP_2FA_MIN_GAP_MS - (now - m_2faLastAttempt);
		CString msg;
		msg.Format(_T("Please wait %u second(s) before the next attempt."), wait / 1000 + 1);
		AfxMessageBox(msg, MB_ICONINFORMATION);
		return false;
	}
	m_2faFirstAttempted = true;
	m_2faLastAttempt = now;
	...
	if (dlg.DoModal() == IDOK) {
		...
		// Echec : journalisation dans l'Event Log Windows
		m_2faFailCount++;
		CString user;
		user.GetEnvironmentVariable(_T("USERNAME"));
		Write2FAFailureEvent(_T("MicroSIP"),
			accessPoint ? (LPCWSTR)*accessPoint : L"acces sensible",
			(LPCWSTR)dlg.code,
			user.IsEmpty() ? NULL : (LPCWSTR)user);

		if (m_2faFailCount >= MICROSIP_2FA_MAX_ATTEMPTS) {
			m_2faLockUntil = now + MICROSIP_2FA_LOCK_MS;
			CString msg;
			msg.Format(_T("Too many invalid 2FA attempts. Access is now locked for 5 minutes."));
			AfxMessageBox(msg, MB_ICONWARNING);
		}
		else {
			CString msg;
			msg.Format(_T("Invalid 2FA code. Access denied. (attempt %d of %d)"), m_2faFailCount, MICROSIP_2FA_MAX_ATTEMPTS);
			AfxMessageBox(msg, MB_ICONWARNING);
		}
	}
	return false;
...
{
	CString accessPoint = _T("Add Account");
	if (!CheckAdminAuth(&accessPoint)) {
		return;
	}
    if (!accountDlg) {
...
{
	CString accessPoint = _T("Edit Account");
	if (!CheckAdminAuth(&accessPoint)) {
		return;
	}
	if (!accountDlg) {
...
void CmainDlg::OnMenuAccountLocalEdit()
{
	if (MACRO_ENABLE_LOCAL_ACCOUNT) {
		CString accessPoint = _T("Edit Local Account");
		if (!CheckAdminAuth(&accessPoint)) {
			return;
		}
		if (!accountDlg) {
...
{
	CString accessPoint = _T("Settings");
	if (!CheckAdminAuth(&accessPoint)) {
		return;
	}
    if (!settingsDlg) {
...
{
	CString accessPoint = _T("New Account Wizard");
	if (!CheckAdminAuth(&accessPoint)) {
		return 0;
	}
	if (!accountDlg) {
...
void CmainDlg::PJCreateRaw()
{
	...
	if (accountSettings.userAgent.IsEmpty()) {
		CString userAgent;
		userAgent.Format(_T("%s/%s"), _T(_GLOBAL_NAME_VISIBLE), _T(_GLOBAL_VERSION));
