...
public:
	...
	bool CheckAdminAuth(const CString* accessPoint = NULL);

	int m_2faFailCount;         // nombre d'echecs 2FA consecutifs (session)
	DWORD m_2faLastAttempt;     // GetTickCount64 de la derniere tentative
	DWORD m_2faLockUntil;       // GetTickCount64 de fin de verrouillage (0 = pas de verrou)
	bool m_2faFirstAttempted;   // vrai apres la 1ere tentative de la session
